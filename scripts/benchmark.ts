/**
 * Benchmark — measures parse and roundtrip times, saves results to docs/benchmark-results.md.
 * Always exits 0.
 */
...
async function main() {
  ...
  console.log(`✅ ${results.at(-1)!.label}: avg ${fmt(r1.avg)}`);
  ...
  if (complexPath) {
    ...
    console.log(`✅ ${results.at(-1)!.label}: avg ${fmt(r2.avg)}`);
  } else {
    console.log('⚠️  No complex document found, skipping');
  }
  ...
  console.log(`✅ ${results.at(-1)!.label}: avg ${fmt(r3.avg)}`);
  ...
  if (complexPath) {
    ...
    console.log(`✅ ${results.at(-1)!.label}: avg ${fmt(r4.avg)}`);
  }
  ...
  console.log(`\n📊 Report saved to docs/benchmark-results.md`);
}
